...
An element whose content is outside scope of CSS formatting model is a replaced element. In other words, content of replaced elements cannot be styled because they're from outside document source. For example, the content of the HTML `img` element is replaced by its `src` attribute's value, the content of input field is entered by user. Typical replaced elements are `img`, `object`, `video` or form elements like `textarea`. `select` and `input`...
...
An element whose content is inside scope of CSS formatting model (e.g., `p` element) is a non-replaced element because its content is found within the element while formatting.
...
<!-- ### 3.7. Block container element

Elements, which generate [block container boxes](#block-container-box), are called **block container elements**. -->
...
Box, which is both block-level box and block container box, is called block box. For example:
...
{% endhighlight %}
    ...
    <div class="col col-x-6 padding-clear-right">
        <iframe name="Anomymous block boxes example" src="/iframes/anonymous-block-boxes.html" width="100%"></iframe>
    </div>
</figure>
...
Flow of an element X is a set consisting of X and all in-flow elements whose <u>nearest out-of-flow ancestor</u> is X. For example:
...
{% endhighlight %}
  ...
  <div class="col col-x-6 padding-clear-right">
      <iframe name="Box flow example" src="/iframes/css-flow.html" width="100%"></iframe>
  </div>
</figure>

The flow of the `#box` div is the div itself, plus the `p` and the `span` elements because the `#box` div is out-of-flow (floated) and the `p` is a valid in-flow element. -->
